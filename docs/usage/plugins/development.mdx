...
A OpenChat plugin consists of the following components:

1. **Plugin Index**: Used to display basic information about the plugin, including the plugin name, description, author, version, and a link to the plugin manifest. The official plugin index can be found at [open-chat-plugins](https://github.com/openhub/open-chat-plugins). If you want to publish a plugin to the official plugin marketplace, you need to [submit a PR](https://github.com/openhub/open-chat-plugins/pulls) to this repository.
...
This section will introduce how to add and use a custom plugin in OpenChat.

<Steps>
  ### Create and Launch Plugin Project

You need to first create a plugin project locally, you can use the template we have prepared [open-chat-plugin-template][open-chat-plugin-template-url]

```bash
$ git clone https://github.com/openhub/chat-plugin-template.git
...
### Add Local Plugin in OpenChat Role Settings

Next, go to OpenChat, create a new assistant, and go to its session settings page:

{' '}

<Image
  alt="Add Local Plugin in OpenChat Role Settings S1"
  height={483}
  inStep
...
<Image
  alt="Add Local Plugin in OpenChat Role Settings S2"
  height={483}
  inStep
...
<Image
  alt="Add Local Plugin in OpenChat Role Settings S3"
  height={483}
  inStep
...
<Image
  alt="Add Local Plugin in OpenChat Role Settings S4"
  height={483}
  inStep
...
After completing these operations, you have understood the basic process of adding custom plugins and using them in OpenChat.
...
3. `ui`: This field contains information about the plugin's user interface, indicating from which address OpenChat loads the plugin's front-end interface. Since OpenChat plugin interface loading is implemented based on iframes, the height and width of the plugin interface can be specified as needed.
4. `gateway`: This field specifies the gateway for OpenChat to query the plugin's API interface. OpenChat's default plugin gateway is a cloud-based service, and requests for custom plugins need to be sent to a locally launched service. Remote calls to a local address are generally not feasible. The `gateway` field solves this problem. By specifying the gateway in the manifest, OpenChat will send plugin requests to this address, and the local gateway address will dispatch requests to the local plugin service. Published online plugins do not need to specify this field.
...
The [open-chat-plugin-template][open-chat-plugin-template-url] template project uses Next.js as the development framework, and its core directory structure is as follows:
...
For the Edge Runtime, we provide the `createErrorResponse` method in `@open/chat-plugin-sdk` to quickly return error responses. Currently, the provided error types are detailed in: [PluginErrorType][plugin-error-type-url].
...
Since the default plugin gateway for OpenChat is a cloud service `/api/plugins`, the cloud service sends requests to the address specified in the manifest's `api.url` to solve cross-origin issues.

For custom plugins, plugin requests need to be sent to the local service. Therefore, by specifying the gateway in the manifest ([http://localhost:3400/api/gateway](http://localhost:3400/api/gateway)), OpenChat> will directly request this address, and then only the corresponding gateway needs to be created at that address.

```ts
import { createOpenChatPluginGateway } from '@open/chat-plugins-gateway';

export const config = {
  runtime: 'edge',
};

export default createOpenChatPluginGateway();
```

[`@open/chat-plugins-gateway`](https://github.com/openhub/chat-plugins-gateway) contains the implementation of the plugin gateway in OpenChat [here](https://github.com/openhub/open-chat/blob/main/src/pages/api/plugins.api.ts). You can use this package directly to create a gateway, allowing OpenChat to access the local plugin service.

### Plugin UI Interface

The custom UI interface for plugins is optional. For example, the official plugin [Web Content Extraction](https://github.com/openhub/chat-plugin-web-crawler) does not have a corresponding user interface.
...
If you want to display richer information in plugin messages or include some interactive operations, you can customize a user interface for the plugin. For example, the following image shows the user interface for the [Search Engine](https://github.com/openhub/chat-plugin-search-engine) plugin.
...
OpenChat implements the loading of plugin UI through `iframe` and uses `postMessage` to communicate with the plugin. Therefore, the implementation of the plugin UI is consistent with regular web development. You can use any frontend framework and development language you are familiar with.
...
In the template we provide, we use React + Next.js + [antd](https://ant.design/) as the frontend interface framework. You can find the implementation of the user interface in [`src/pages/index.tsx`](https://github.com/openhub/chat-plugin-template/blob/main/src/pages/index.tsx).

As for plugin communication, we provide relevant methods in [`@open/chat-plugin-sdk`](https://github.com/openhub/chat-plugin-sdk) to simplify communication between the plugin and OpenChat. You can actively retrieve the current message data from OpenChat using the `fetchPluginMessage` method. For detailed information about this method, see: [fetchPluginMessage][fetch-plugin-message-url].

```tsx
import { fetchPluginMessage } from '@open/chat-plugin-sdk';
...
const Render = memo(() => {
  ...
  useEffect(() => {
    // Retrieve the current plugin message from OpenChat
...
If you want more people to use your plugin, feel free to [submit it for listing](https://github.com/openhub/open-chat-plugins) on the plugin marketplace.

[![][submit-plugin-shield]][submit-plugin-url]

### Plugin Shield

[![open-chat-plugin](https://img.shields.io/badge/%F0%9F%A4%AF%20%26%20%F0%9F%A7%A9%20OpenHub-Plugin-95f3d9?labelColor=black&style=flat-square)](https://github.com/openhub/open-chat-plugins)
...
- **📘 Pluging SDK Documentation**: [https://chat-plugin-sdk.openhub.com](https://chat-plugin-sdk.openhub.com)
- **🚀 chat-plugin-template**: [https://github.com/openhub/chat-plugin-template](https://github.com/openhub/chat-plugin-template)
- **🧩 chat-plugin-sdk**: [https://github.com/openhub/chat-plugin-sdk](https://github.com/openhub/chat-plugin-sdk)
- **🚪 chat-plugin-gateway**: [https://github.com/openhub/chat-plugins-gateway](https://github.com/openhub/chat-plugins-gateway)
- **🏪 open-chat-plugins**: [https://github.com/openhub/open-chat-plugins](https://github.com/openhub/open-chat-plugins)

[fetch-plugin-message-url]: https://github.com/openhub/chat-plugin-template
[open-chat-plugin-template-url]: https://github.com/openhub/chat-plugin-template
[manifest-docs-url]: https://chat-plugin-sdk.openhub.com/guides/plugin-manifest
[plugin-error-type-url]: https://github.com/openhub/chat-plugin-template
[submit-plugin-shield]: https://img.shields.io/badge/🧩/🏪_submit_plugin-%E2%86%92-95f3d9?labelColor=black&style=for-the-badge
[submit-plugin-url]: https://github.com/openhub/open-chat-plugins
