...
点击左侧导航栏的「Users Management」，进入用户管理界面，可以为你的组织新建用户，用以登录 OpenChat
...
在部署 OpenChat 时，你需要配置以下环境变量：

| 环境变量 | 类型 | 描述 |
| --- | --- | --- |
| `ENABLE_OAUTH_SSO` | 必选 | 为 OpenChat 启用单点登录 (SSO)。设置为 `1` 以启用单点登录。 |
...
<Callout type={'info'}>
  部署成功后，用户将可以使用 Auth0 中配置的用户通过身份认证并使用 OpenChat。
</Callout>
...
<Callout type={'warning'}>
  配置社交登录服务默认会允许所有人通过认证，这可能会导致 OpenChat 被外部人员滥用。
</Callout>
