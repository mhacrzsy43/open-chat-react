...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest OpenChat image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running OpenChat Container

Assuming the OpenChat container is named `open-chat`, use the following commands to stop and remove the currently running OpenChat container:
...
### Pull the Latest OpenChat Image

Use the following command to pull the latest Docker image for OpenChat:
...
Redeploy the OpenChat container using the newly pulled image:
...
No need to worry, you won't. All of OpenChat's chat records are stored in your local browser. Therefore, when redeploying OpenChat using Docker, your chat records will not be lost.
