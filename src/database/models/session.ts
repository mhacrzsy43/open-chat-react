...
class _SessionModel extends BaseModel {
  ...
  async query({
    pageSize = 9999,
    current = 0,
  }: { current?: number; pageSize?: number } = {}): Promise<OpenSessions> {
   ...
   * @param group
   */
  async querySessionsByGroupId(group: SessionGroupId): Promise<OpenSessions> {
    const items: DBModel<DB_Session>[] = await this.table
      .where("group")
      .equals(group)
   ...
   */
  async queryByKeyword(keyword: string): Promise<OpenSessions> {
    if (!keyword) return [];
  ...
  async getPinnedSessions(): Promise<OpenSessions> {
    const items: DBModel<DB_Session>[] = await this.table
      .where("pinned")
      .equals(1)
      .reverse()
  ...
  async batchCreate(sessions: OpenAgentSession[]) {
  ...
  async updateConfig(id: string, data: DeepPartial<OpenAgentConfig>) {
    const session = await this.findById(id);
    if (!session) return;
  ...
  private mapToDB_Session(session: OpenAgentSession): DBModel<DB_Session> {
    return {
      ...session,
