...
describe("SessionModel", () => {
  let sessionData: Partial<OpenAgentSession>;
  ...
  beforeEach(() => {
    ...
    sessionData = {
      ...
      // ... other properties based on OpenAgentSession
    };
  });
  ...
  describe("batchCreate", () => {
    it("should batch create session records", async () => {
      ...
      const results = await SessionModel.batchCreate(
        sessionsToCreate as OpenAgentSession[]
      );
  ...
  describe("queryWithGroups", () => {
    it("should query sessions with groups", async () => {
      ...
      expect(sessionsWithGroups.all[0]).toEqual(
