...
describe("SessionService", () => {
  ...
  describe("createNewSession", () => {
    it("should create a new session and return its id", async () => {
      // Setup
      const sessionType = OpenSessionType.Agent;
      const defaultValue = {
        meta: { title: "New Session" },
      } as Partial<OpenAgentSession>;
    ...
    it("should throw an error if session creation fails", async () => {
      ...
      // Execute & Assert
      await expect(
        sessionService.createNewSession(sessionType, defaultValue)
  ...
  describe("updateSessionGroupId", () => {
    it("should update the group of a session", async () => {
      ...
      // Execute
      const result = await sessionService.updateSessionGroupId(
        mockSessionId,
        groupId
      );
  ...
  describe("updateSessionMeta", () => {
    it("should update the meta of a session", async () => {
      ...
      // Execute
      const result = await sessionService.updateSessionMeta(
        mockSessionId,
        newMeta
      );
  ...
  describe("updateSessionConfig", () => {
    it("should update the config of a session", async () => {
      // Setup
      const newConfig = { compressThreshold: 2 } as OpenAgentConfig;
  ...
  describe("duplicateSession", () => {
    it("should duplicate a session and return its id", async () => {
      ...
      // Execute
      const duplicatedSessionId = await sessionService.duplicateSession(
        mockSessionId,
        newTitle
      );
  ...
  describe("updateSessionPinned", () => {
    it("should update the pinned status of a session", async () => {
      // Setup
      const pinned = true;
      (SessionModel.updatePinned as Mock).mockResolvedValue({
        ...mockSession,
        pinned,
      });

      // Execute
      const result = await sessionService.updateSessionPinned(
        mockSessionId,
        pinned
      );

      // Assert
      expect(SessionModel.updatePinned).toHaveBeenCalledWith(
        mockSessionId,
        pinned
      );
