...
class PluginService {
   ...
   */
  getPluginList = async (): Promise<OpenChatPluginsMarketIndex> => {
  ...
  getPluginManifest = async (
  ...
  ): Promise<OpenChatPluginManifest> => {
    // 1. valid plugin
    if (!url) {
    ...
    let data = await this._fetchJSON<OpenChatPluginManifest>(url, useProxy);

    // @ts-ignore
    // if there is a description_for_model, it is an OpenAI plugin
    // we need convert to open plugin
    if (data["description_for_model"]) {
      data = this.convertOpenAIManifestToOpenManifest(data as any);
    }
  ...
  getInstalledPlugins = () => {
    return PluginModel.getList() as Promise<OpenTool[]>;
  };
  ...
  async createCustomPlugin(customPlugin: OpenToolCustomPlugin) {
    return PluginModel.create({ ...customPlugin, type: "customPlugin" });
  }

  async updatePlugin(id: string, value: OpenToolCustomPlugin) {
    return PluginModel.update(id, value);
  }
  async updatePluginManifest(id: string, manifest: OpenChatPluginManifest) {
  ...
  private convertOpenAIManifestToOpenManifest = (
    data: OpenAIPluginManifest
  ): OpenChatPluginManifest => {
    const manifest: OpenChatPluginManifest = {
      api: [],
