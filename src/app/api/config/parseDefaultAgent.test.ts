...
describe('parseAgentConfig', () => {
  describe('single functional feature', () => {
    ...
    it('parses array values with commas correctly', () => {
      const envStr = 'plugins=search-engine,open-image-designer';
      const expected = { plugins: ['search-engine', 'open-image-designer'] };
      expect(parseAgentConfig(envStr)).toEqual(expected);
    });
  ...
  describe('complex environment', () => {
    it.skip('parses a complete environment variable string correctly', () => {
      const envStr =
        'model=gpt-4-1106-preview;params.max_tokens=300;plugins=search-engine,open-image-designer';
      const expected = {
        model: 'gpt-4-1106-preview',
        params: { max_tokens: 300 },
        plugins: ['search-engine', 'open-image-designer'],
      };
      expect(parseAgentConfig(envStr)).toEqual(expected);
    });
    ...
    it('parses a complex environment variable string correctly', () => {
      const envStr =
        'model=gpt-4-1106-preview;params.max_tokens=300;params.temperature=0.7;plugins=search-engine,open-image-designer;tts.voice.openai=english-voice';
      const expected = {
        model: 'gpt-4-1106-preview',
