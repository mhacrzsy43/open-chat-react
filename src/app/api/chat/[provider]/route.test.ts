...
import { OpenRuntimeAI } from "@/libs/agent-runtime";
...
describe("POST handler", () => {
  describe(" init chat model", () => {
    it("should initialize AgentRuntime correctly with valid authorization", async () => {
      ...
      const mockRuntime: OpenRuntimeAI = { baseURL: "abc", chat: vi.fn() };
  ...
  describe("chat", () => {
    it("should correctly handle chat completion with valid payload", async () => {
      ...
      expect(AgentRuntime.prototype.chat).toHaveBeenCalledWith(
        mockChatPayload,
        {
          provider: "test-provider",
        }
