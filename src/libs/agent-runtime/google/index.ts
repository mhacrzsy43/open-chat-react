...
import { AgentRuntimeErrorType, IOpenAgentRuntimeErrorType } from "../error";
import {
...
export class OpenGoogleAI implements OpenRuntimeAI {
  ...
  async chat(payload: ChatStreamPayload, options?: ChatCompetitionOptions) {
    try {
      const { contents, model } = this.buildGoogleMessages(
        payload.messages,
        payload.model
      );
  ...
  private parseErrorMessage(message: string): {
    error: any;
    errorType: IOpenAgentRuntimeErrorType;
  } {
    const defaultError = {
      error: { message },
