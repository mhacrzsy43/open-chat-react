...
import { OpenRuntimeAI } from "../BaseAI";
...
interface OpenOpenAIOptions extends ClientOptions {
  azureOptions?: {
    apiVersion?: string;
    model?: string;
  };
...
export class OpenOpenAI implements OpenRuntimeAI {
  private client: OpenAI;

  constructor(options: OpenOpenAIOptions) {
    if (!options.apiKey)
      throw AgentRuntimeError.createError(AgentRuntimeErrorType.NoOpenAIAPIKey);

    if (options.useAzure) {
      this.client = OpenOpenAI.initWithAzureOpenAI(options);
    } else {
  ...
  async chat(payload: ChatStreamPayload, options?: ChatCompetitionOptions) {
    ...
    try {
      ...
      // refs: https://github.com/openhub/open-chat/issues/842
  ...
  static initWithAzureOpenAI(options: OpenOpenAIOptions) {
    ...
    const baseURL = urlJoin(
      endpoint,
      `/openai/deployments/${model.replace(".", "")}`
    );
