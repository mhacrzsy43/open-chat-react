...
import { ChatStreamCallbacks, OpenOpenAI } from "@/libs/agent-runtime";
...
import { OpenZhipuAI } from "./index";

// Mock相关依赖
vi.mock("./authToken");

describe("OpenZhipuAI", () => {
  beforeEach(() => {
  ...
  describe("fromAPIKey", () => {
    it("should correctly initialize with an API key", async () => {
      const openZhipuAI = await OpenZhipuAI.fromAPIKey({
        apiKey: "test_api_key",
      });
      expect(openZhipuAI).toBeInstanceOf(OpenZhipuAI);
      expect(openZhipuAI.baseURL).toEqual(
    ...
    it("should throw an error if API key is invalid", async () => {
      ...
      try {
        await OpenZhipuAI.fromAPIKey({ apiKey: "asd" });
      } catch (e) {
  ...
  describe("chat", () => {
    let instance: OpenZhipuAI;

    beforeEach(async () => {
      instance = await OpenZhipuAI.fromAPIKey({
        apiKey: "test_api_key",
      });
    ...
    describe("Error", () => {
      ...
      it("should throw AgentRuntimeError with NoOpenAIAPIKey if no apiKey is provided", async () => {
        try {
          await OpenZhipuAI.fromAPIKey({ apiKey: "" });
        } catch (e) {
          expect(e).toEqual({ errorType: "InvalidZhipuAPIKey" });
        }
      });
