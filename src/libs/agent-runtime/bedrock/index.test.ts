...
import { OpenBedrockAI } from "./index";
...
let instance: OpenBedrockAI;

beforeEach(() => {
  instance = new OpenBedrockAI({
    region: "us-west-2",
...
describe("OpenBedrockAI", () => {
  describe("init", () => {
    it("should correctly initialize with AWS credentials", async () => {
      const instance = new OpenBedrockAI({
        region: "us-west-2",
        accessKeyId: "test-access-key-id",
        accessKeySecret: "test-access-key-secret",
      });
      expect(instance).toBeInstanceOf(OpenBedrockAI);
    });
  });
