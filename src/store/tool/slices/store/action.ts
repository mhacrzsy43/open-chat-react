import { OpenChatPluginsMarketIndex } from "@open/chat-plugin-sdk";
...
export interface PluginStoreAction {
  ...
  loadPluginStore: () => Promise<OpenChatPluginsMarketIndex>;
  refreshPlugins: () => Promise<void>;
  uninstallPlugin: (identifier: string) => Promise<void>;

  updateInstallLoadingState: (key: string, value: boolean | undefined) => void;
  useFetchInstalledPlugins: () => SWRResponse<OpenTool[]>;
  useFetchPluginStore: () => SWRResponse<OpenChatPluginsMarketIndex>;
}
...
> = (set, get) => ({
  ...
  useFetchInstalledPlugins: () =>
    useSWR<OpenTool[]>(INSTALLED_PLUGINS, pluginService.getInstalledPlugins, {
      onSuccess: (data) => {
        set(
  ...
  useFetchPluginStore: () =>
    useSWR<OpenChatPluginsMarketIndex>(
      "loadPluginStore",
      get().loadPluginStore
    ),
});
