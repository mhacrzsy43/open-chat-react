...
import { OpenSessionType } from "@/types/session";
...
describe("SessionAction", () => {
  ...
  describe("createSession", () => {
    it("should create a new session and switch to it", async () => {
      ...
      vi.mocked(sessionService.createNewSession).mockResolvedValue(
        newSessionId
      );
      ...
      expect(call[0]).toEqual(OpenSessionType.Agent);
  ...
  describe("duplicateSession", () => {
    it("should duplicate a session and switch to the new one", async () => {
      ...
      vi.mocked(sessionService.duplicateSession).mockResolvedValue(
        duplicatedSessionId
      );
      ...
      expect(sessionService.duplicateSession).toHaveBeenCalledWith(
        sessionId,
        undefined
      );
    });
  ...
  describe("pinSession", () => {
    it("should pin a session when pinned is true", async () => {
      ...
      expect(sessionService.updateSessionPinned).toHaveBeenCalledWith(
        sessionId,
        true
      );
      expect(mockRefresh).toHaveBeenCalled();
  ...
  describe("updateSessionGroupId", () => {
    it("should update the session group and refresh the list", async () => {
      ...
      expect(sessionService.updateSessionGroupId).toHaveBeenCalledWith(
        sessionId,
        groupId
      );
      expect(mockRefresh).toHaveBeenCalled();
