...
describe("currentSessionSafe", () => {
  ...
  it("should return initOpenSession when currentSession(s) returns undefined", () => {
    expect(
      sessionSelectors.currentSessionSafe({ sessions: {} } as any)
    ).toEqual(initOpenSession);
  });
...
describe("getSessionById", () => {
  ...
  it("should return initOpenSession when the session with the specified id does not exist", () => {
    expect(sessionSelectors.getSessionById("3")(s)).toEqual(initOpenSession);
  });
});
