...
export interface SessionAction {
   ...
   */
  createSession: (
    session?: DeepPartial<OpenAgentSession>,
...
> = (set, get) => ({
  ...
  createSession: async (agent, isSwitchSession = true) => {
    ...
    const newSession: OpenAgentSession = merge(defaultAgent, agent);

    const id = await sessionService.createNewSession(
      OpenSessionType.Agent,
      newSession
    );
  ...
  useFetchSessions: () =>
    useClientDataSWR<ChatSessionList>(
      ...
      {
        onSuccess: (data) => {
          // 由于 https://github.com/openhub/open-chat/pull/541 的关系
  ...
  useSearchSessions: (keyword) =>
    useSWR<OpenSessions>(keyword, sessionService.searchSessions, {
      onSuccess: (data) => {
        set(
          { searchSessions: data },
          false,
          n("useSearchSessions(success)", data)
        );
      },
