...
English | [简体中文](https://github.com/openhub/open-chat/wiki/Upstream-Sync.zh-CN)
...
Upgrading the Docker deployment version is very simple, just redeploy the latest image of OpenChat. Here are the instructions to perform these steps:

1. Stop and delete the currently running OpenChat container (assuming the name of the OpenChat container is `open-chat`):

```fish
docker stop open-chat
docker rm open-chat
```

2. Pull the latest Docker image of OpenChat:

```fish
docker pull openhub/open-chat
```

3. Redeploy the OpenChat container using the newly pulled image:
...
>
> Don't worry, all of OpenChat's chat history is stored in your local browser. Therefore, when you redeploy OpenChat using Docker, your chat history will not be lost.
