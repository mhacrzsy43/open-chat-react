...
[English](https://github.com/openhub/open-chat/wiki/Upstream-Sync) | 简体中文
...
Docker 部署版本的升级非常简单，只需要重新部署 OpenChat 的最新镜像即可。 以下是执行这些步骤所需的指令：

1. 停止并删除当前运行的 OpenChat 容器（假设 OpenChat 容器的名称是 `open-chat`）：
...
2. 拉取 OpenChat 的最新 Docker 镜像：

```fish
docker pull openhub/open-chat
```

3. 使用新拉取的镜像重新部署 OpenChat 容器：
...
> 重新部署的话，我本地的聊天记录会丢失吗？
>
> 放心，OpenChat 的聊天记录全部都存储在你的本地浏览器中。因此使用 Docker 重新部署 OpenChat 时，你的聊天记录并不会丢失。
