...
The implementation of OpenChat's large model AI mainly relies on OpenAI's API, including the core conversation API on the backend and the integrated API on the frontend. Next, we will introduce the implementation approach and code for the backend and frontend separately.
...
export const fetchChatModel = (
  ...
  const payload = merge(
    {
      model: initialOpenAgentConfig.model,
      stream: true,
      ...initialOpenAgentConfig.params,
    },
    params
  );
...
The above is the core implementation of the OpenChat session API. With an understanding of these core codes, further expansion and optimization of OpenChat's AI functionality can be achieved.
